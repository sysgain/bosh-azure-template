...
# Pivotal/Apigee/Concourse - Solution Template (TBD)
<a href="">
...
**Pivotal/Apigee/Concourse - Solution Template (TBD)** launches a stack based around Pivotal Cloud Foundry, the leading cloud application platform for running Java microservices. This solution template will stand up a fully integrated Pivotal Cloud Foundry stack along with Concourse CI, Azure Meta Service Broker, Apigee Edge Gateway and other services to rapidly build and deploy Java-based applications & microservices.  
...
## Prerequisites
* Azure user account with Contributor/Admin Role
* Sufficiently high quota limits (Recommended: 50 cores) on your Azure account. Installing Pivotal Cloud Foundry® requires more than the default 20 cores.
	*Please see this [link](https://blogs.msdn.microsoft.com/girishp/2015/09/20/increasing-core-quota-limits-in-azure/) for instructions on requesting a core quota increase.
*Install either the Azure CLI or Azure PowerShell on your machine, using the instructions [here](https://azure.microsoft.com/en-us/documentation/articles/powershell-install-configure/). Create an Azure Service Principal (TENANT-ID, CLIENT-ID, CLIENT-SECRET):
* Use the Instructions [here](https://github.com/danhigham/azure-sp-tool) to generate Azure Service Principal file
...
<a href="">
<img src="https://raw.githubusercontent.com/sysgain/pivotal/master/pivotal-P2P-Architecture.jpg"/>
</a>
...
The fly tool is a command line interface to Concourse. It is used for a number of tasks from connecting to a shell in one of your build's containers to uploading new pipeline configuration into a running Concourse.
